...
local function CreateDropdownOption(Name, Order)
    ...
    Option.MouseButton1Click:Connect(function()
        ...
        if _G.DRE_EggCheckPremium then
            local List = {}
            if SelectedRarities.Secret then table.insert(List, "Secret") end
            if SelectedRarities.Eternal then table.insert(List, "Eternal") end
            if SelectedRarities.Divine then table.insert(List, "Divine") end
            _G.DRE_EggCheckPremium.SetRarities(List)
        end

        -- ✅ Update FarmingManager
        if _G.DRE_FarmingManager then
            local List = {}
            if SelectedRarities.Secret then table.insert(List, "Secret") end
            if SelectedRarities.Eternal then table.insert(List, "Eternal") end
            if SelectedRarities.Divine then table.insert(List, "Divine") end
            _G.DRE_FarmingManager.SetRarities(List)
        end
...
local function ToggleFarm()
    if not _G.DRE_FarmingManager then
        warn("[DRE] FarmingManager not loaded!")
        return
    end
    ...
    if FarmEnabled then
        ...
        _G.DRE_FarmingManager.Enable()
    else
        FarmButton.BackgroundColor3 = Color3.fromRGB(28, 29, 39)
        FarmStroke.Color = Color3.fromRGB(200, 200, 220)
        _G.DRE_FarmingManager.Disable()
    end
end
...
task.spawn(function()
    task.wait(1)
    if _G.DRE_FarmingManager then
        local State = _G.DRE_FarmingManager.IsEnabled()
        FarmEnabled = State
...
task.spawn(function()
    while task.wait(1) do
        if _G.DRE_FarmingManager then
            local CurrentState = _G.DRE_FarmingManager.IsEnabled()
            ...
            if CurrentState ~= UIState then
                ...
                print("[DRE] Farming UI Sync | State: " .. tostring(CurrentState))
            end
        end
    end
end)
...
-- ==================================================
_G.DRE_RefreshFarmingUI = function()
    if _G.DRE_FarmingManager then
        local State = _G.DRE_FarmingManager.IsEnabled()
        FarmEnabled = State
        FarmCheck.Visible = State
        ...
        print("[DRE] Farming Tab UI Refreshed | State: " .. tostring(State))
    end
end
