...
local function UpdateGetEggBox(Icon, Name, Rate, EggId)
    ...
    GetEggRate.Text = "$" .. (_G.DRE_AutoFarm and _G.DRE_AutoFarm.FormatMoney(Rate or 0) or tostring(Rate or 0)) .. "/s"
    SelectedEggId = EggId
...
local function ToggleGetEgg()
    ...
    if GetEggEnabled then
        ...
        if _G.DRE_AutoFarm then
            _G.DRE_AutoFarm.StartTeleport()
        end
    else
        GetEggCheckButton.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        GetEggCheckButton.BackgroundTransparency = 0.85
...
local function CreateEggEntry(EggData)
    ...
    RateLabel.Text = "$" .. (_G.DRE_AutoFarm and _G.DRE_AutoFarm.FormatMoney(EggData.EarningRate) or tostring(EggData.EarningRate)) .. "/s"
    ...
    SelectButton.MouseButton1Click:Connect(function()
        ...
        if _G.DRE_AutoFarm then
            _G.DRE_AutoFarm.SelectEgg(EggData)
        end
    end)
...
local function RefreshEggList()
    ...
    if not _G.DRE_AutoFarm then return end
    ...
    local Eggs = _G.DRE_AutoFarm.ScanEggs()
