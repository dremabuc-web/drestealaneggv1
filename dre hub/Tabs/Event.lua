...
ManagerButton.MouseButton1Click:Connect(function()
    if not _G.DRE_ManagerDrone then
        warn("[DRE] ManagerDrone not loaded!")
        return
    end

    local NewState = not _G.DRE_ManagerDrone.IsEnabled()
    UpdateManagerUI(NewState)

    -- ✅ Call Enable/Disable (មិន Save Config)
    if NewState then
        _G.DRE_ManagerDrone.Enable()
    else
        _G.DRE_ManagerDrone.Disable()
    end
end)
...
task.spawn(function()
    task.wait(1)
    if _G.DRE_ManagerDrone then
        local State = _G.DRE_ManagerDrone.IsEnabled()
...
-- ✅ REFRESH FUNCTION
--==================================================
_G.DRE_RefreshEventUI = function()
    if _G.DRE_ManagerDrone then
        local State = _G.DRE_ManagerDrone.IsEnabled()
        UpdateManagerUI(State)
        print("[DRE] Event Tab UI Refreshed | State: " .. tostring(State))
    end
end
...
task.spawn(function()
    while task.wait(1) do
        if _G.DRE_ManagerDrone then
            local CurrentState = _G.DRE_ManagerDrone.IsEnabled()
            local UIState = ManagerCheck.Visible

            if CurrentState ~= UIState then
                UpdateManagerUI(CurrentState)
                print("[DRE] Event UI Sync | State: " .. tostring(CurrentState))
            end
        end
    end
end)
