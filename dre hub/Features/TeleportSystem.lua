-- ==================================================
-- DRE HUB - TELEPORT SYSTEM (DUAL MODE + DUAL OPTION)
...
if not CollectEvent then
    warn("[DRE] CollectEvent not found")
    return
end

print("[DRE] TeleportSystem: CollectEvent OK")
...
local function CleanupMovers()
    ...
    if Root then
        for _, Child in ipairs(Root:GetChildren()) do
            if Child.Name == "DREBV" or Child.Name == "DREBG" then
...
local function TeleportToTarget(TargetPos, Callback)
    if CurrentMethod == "InstantTeleport" then
        print("[DRE] Instant TP to Target")
        InstantFlyTP(TargetPos, Callback)
    else
        print("[DRE] FlyTP to Target (Shot TP)")
...
local function AutoStop()
    ...
    print("[DRE] TeleportSystem: Auto Stop")
end
...
local function StartProcess()
    ...
    print("[DRE] FlyTP to First Egg (Shot TP)")
...
local function FullReset()
    ...
    print("[DRE] TeleportSystem: Full Reset")
end
...
local function Enable()
    ...
    if not CollectEvent then warn("[DRE] CollectEvent not found") return end
    if not TARGET_UID then warn("[DRE] No Target ID") return end

    FullReset()
    StartProcess()

    print("[DRE] TeleportSystem: ON | Method: " .. CurrentMethod)
end

local function Disable()
    FullReset()
    print("[DRE] TeleportSystem: OFF")
end

local function SetTargetId(Id)
    TARGET_UID = Id
    print("[DRE] TeleportSystem Target ID: " .. tostring(Id))
end
...
local function SetSpeed(Value)
    ...
    FLY_SPEED = Value
    RETURN_SPEED = Value
    print("[DRE] TeleportSystem Speed: " .. tostring(Value))
end
...
local function SetMethod(Method)
    ...
    print("[DRE] TeleportSystem Method: " .. CurrentMethod)
end
