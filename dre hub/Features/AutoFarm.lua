...
local function EnableAutoFarm()
    ...
    print("[DRE] Auto Farm: ON")
end

local function DisableAutoFarm()
    AutoFarmEnabled = false
    print("[DRE] Auto Farm: OFF")
end
...
local function SelectEgg(EggData)
    SelectedEgg = EggData
    print("[DRE] Selected Egg: " .. EggData.DisplayName .. " ($" .. FormatMoney(EggData.EarningRate) .. "/s)")
end
...
local function StartTeleport()
    if not SelectedEgg then
        warn("[DRE] No Egg Selected")
        return
    end

    local Method = _G.DRE_SelectedMethod or "TeleportFly"
    local Speed = _G.DRE_TeleportSpeed or 300

    print("[DRE] Start Teleport | Method: " .. Method .. " | Speed: " .. tostring(Speed) .. " | Target: " .. SelectedEgg.Id)

    if _G.DRE_TeleportSystem then
        _G.DRE_TeleportSystem.SetMethod(Method)
        _G.DRE_TeleportSystem.SetSpeed(Speed)
        _G.DRE_TeleportSystem.SetTargetId(SelectedEgg.Id)
        _G.DRE_TeleportSystem.Enable()
    end
end
