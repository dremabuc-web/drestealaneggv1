...
local function ForceStopAll()
    ...
    if _G.DRE_AttackDrone then
        pcall(function() _G.DRE_AttackDrone.Stop() end)
    end
    if _G.DRE_AFKSystem then
        pcall(function() _G.DRE_AFKSystem.Disable() end)
    end
end
...
local function SwitchAFKToAttack()
    ...
    if _G.DRE_AFKSystem then
        TreadmillPos = _G.DRE_AFKSystem.GetMyTreadmillPos()
    end

    if not TreadmillPos and _G.DRE_AFKSystem then
        local _, Treadmill = _G.DRE_AFKSystem.FindMyPlotAndTreadmill()
        if Treadmill then
    ...
    if not TreadmillPos then
        ...
        if _G.DRE_AFKSystem then
            _G.DRE_AFKSystem.Disable()
        end
        task.wait(0.5)
        if _G.DRE_AttackDrone then
            _G.DRE_AttackDrone.Start()
        end
        return
    end
    ...
    _G.DRE_AFKSystem.JumpOutTreadmill(TreadmillPos, function()
...
local function MainLoop()
    while ManagerEnabled do
        ...
        if EventNotActive then
            if _G.DRE_AttackDrone and _G.DRE_AttackDrone.IsEnabled() then
                print("[ManagerDrone] Event Not Active → Stop Attack")
                _G.DRE_AttackDrone.Stop()
            end

            if _G.DRE_AFKSystem and not _G.DRE_AFKSystem.IsEnabled() then
                print("[ManagerDrone] Event Not Active → AFK System")
                _G.DRE_AFKSystem.Enable()
            end
        -- ==================================================
        ...
        elseif EventActive then
            if _G.DRE_AFKSystem and _G.DRE_AFKSystem.IsEnabled() then
                print("[ManagerDrone] Event Active → Switch AFK to Attack")
                SwitchAFKToAttack()
            elseif _G.DRE_AttackDrone and not _G.DRE_AttackDrone.IsEnabled() then
