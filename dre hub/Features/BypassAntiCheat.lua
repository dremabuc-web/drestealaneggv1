-- ==================================================
-- DRE HUB | FEATURE | Bypass Anti Cheat
...
local function RunBypassAntiCheat()
    ...
    if not OldHumanoid then
        warn("[DRE] Humanoid not found")
        return
    end

    print("========================================")
    print("[DRE] START HUMANOID REPLACE")
    print("========================================")
    ...
    if not NewHumanoid then
        warn("[DRE] Failed to clone Humanoid")
        return
    end
    ...
    if not NewHumanoid.Parent then
        warn("[DRE] New Humanoid was removed")
        return
    end

    print("[DRE] New Humanoid:", NewHumanoid)

    --==================================================
    -- UPDATE CHARACTER SYSTEM
    --==================================================
    if _G.DRE_CharacterSystem then
        _G.DRE_CharacterSystem.CurrentHumanoid = NewHumanoid
        _G.DRE_CharacterSystem.CurrentRoot = Character:FindFirstChild("HumanoidRootPart")
        print("[DRE] CharacterSystem Updated with New Humanoid")
    end
    ...
    local function RefreshControls()
        ...
        if not PlayerScripts then
            warn("[DRE] PlayerScripts not found")
            return
        end
        local PlayerModule = PlayerScripts:FindFirstChild("PlayerModule")
        if not PlayerModule then
            warn("[DRE] PlayerModule not found")
            return
        end
        ...
        if not Success or not Module then
            warn("[DRE] Failed to require PlayerModule")
            return
        end
        local Controls
        pcall(function()
        ...
        task.wait()
        print("[DRE] Controls Humanoid:", Controls.humanoid)
        print("[DRE] Same Humanoid:", Controls.humanoid == NewHumanoid)
    end
    ...
    if _G.DRE_CharacterSystem then
        task.spawn(function()
            task.wait(0.5)
            _G.DRE_CharacterSystem:RestartAllFeatures()
        end)
    end

    print("")
    print("========================================")
    print("[DRE] HUMANOID REPLACE + ANTI DEATH COMPLETE")
    print("========================================")
end
